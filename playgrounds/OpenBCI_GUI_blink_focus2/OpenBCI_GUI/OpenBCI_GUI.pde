...
void initSystem() {
  ...
  accBuff = new float[auxchan][dataBuffX.length];
  ...
  for (int i=0; i<nDataBackBuff; i++) { 
    ...
    dataPacketBuff[i] = new DataPacket_ADS1299(nchan, auxchan);
  }
...
int getDataIfAvailable(int pointCounter) {
  ...
  if ( (eegDataSource == DATASOURCE_NORMAL) || (eegDataSource == DATASOURCE_NORMAL_W_AUX) ) {
    ...
    while ( (curDataPacketInd != lastReadDataPacketInd) && (pointCounter < nPointsPerUpdate)) {
      ...
      //DM : moving acc data into the buffer
...
void processNewData() {
  ...
  eegProcessing_user.process(yLittleBuff_uV, dataBuffY_uV, dataBuffY_filtY_uV, fftBuff, accBuff);
...
void serialEvent(Serial port) {
  ...
  if (openBCI.isOpenBCISerial(port)) {
    ...
    if (openBCI.get_isNewDataPacketAvailable()) {
      ...
      //println("Testing" + dataPacketBuff[curDataPacketInd].auxValues);
    }
  } else {
