...
class EEG_Processing_User {
  ...
  public void process(float[][] data_newest_uV, //holds raw EEG data that is new since the last call
    ...
    myAverage_acc = 0;
    for (int i = 0; i < 50; i++) {
      //println( "buffer value at i(" + i + ") is : " + accBuff[Achan][i] + " ... acceptable limit is  " + acceptableLimitUV);
      if (accBuff[Achan][i] <= acceptableLimitAux) { //prevent BIG spikes from effecting the average
        myAverage_acc += accBuff[Achan][i];  //add value to average ... we will soon divide by # of packets
      println( "potato " + accBuff[Achan][i]);  
    }
    }
  ...
  public void draw() {
    ...
    ellipse(3*(width/4)+x_move, (height/4)+y_move, 50, 50);
